...
Anggap bahwa kamu akan membuat tabel daftar mahasiswa matematika dengan beberapa kolom sebagai berikut.
* Nama          $\to$ `nama`    (VARCHAR(30))
* NIM           $\to$ `nim`     (INT(18))
* Asal          $\to$ `asal`    (VARCHAR(30))
* Nomor Telepon $\to$ `no_telp` (INT(12))

bagian yang di block merupakan nama tabel.
